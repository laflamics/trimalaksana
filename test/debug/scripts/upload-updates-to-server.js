...
function getUploadServerUrl() {
  // 🚀 FIX: Selalu pakai Vercel proxy untuk upload (lebih cepat dan stabil)
  const vercelUrl = process.env.VERCEL_URL || process.env.UPDATE_SERVER_VERCEL_URL || 'vercel-proxy-blond-nine.vercel.app';
  
  // Pakai Vercel URL langsung (sudah include https://)
  const url = vercelUrl.startsWith('http') ? vercelUrl : `https://${vercelUrl}`;
  console.error(`🚀 Using Vercel proxy for upload: ${url}`);
  return url;
}
...
function testConnection(serverUrl) {
  return new Promise((resolve) => {
    ...
    const isVercelProxy = url.hostname.includes('vercel.app') || url.hostname.includes('vercel-proxy');
    const isDirectIP = /^\d+\.\d+\.\d+\.\d+$/.test(url.hostname);
    
    // 🚀 FIX: Vercel proxy menggunakan /health endpoint, bukan root
    // Test dengan health endpoint yang pasti ada di Vercel proxy
    const testPath = isVercelProxy ? '/health' : '/';
    const testPort = url.port || (url.protocol === 'https:' ? 443 : 80);
    ...
    const testReq = httpModule.request({
      ...
      path: testPath,
      method: 'GET',
      timeout: 5000, // 5 detik timeout untuk test (Vercel mungkin perlu waktu lebih lama)
      // 🚀 FIX: Vercel proxy menggunakan valid SSL certificate, tidak perlu skip verification
      rejectUnauthorized: !isDirectIP && !isVercelProxy
    }, (res) => {
      // Any response (bahkan 404) berarti server is reachable
      resolve(true);
    });
...
function uploadFile(filePath, serverUrl) {
  return new Promise((resolve, reject) => {
    ...
    // 🚀 FIX: Vercel proxy selalu HTTPS, port 443 (default)
    ...
    if (!port) {
      if (isVercelProxy) {
        port = 443; // Vercel proxy selalu HTTPS
      } else if (isDirectIP) {
    ...
    const options = {
      ...
      // 🚀 FIX: Vercel proxy menggunakan HTTPS, tidak perlu port 8888
      method: 'POST',
      headers: {
      ...
      rejectUnauthorized: !isTailscale && !isVercelProxy,
...
async function uploadFiles() {
  try {
    ...
    // Test connection dulu (timeout 5 detik untuk Vercel proxy)
    const canConnect = await testConnection(serverUrl);
    
    if (!canConnect) {
      // 🚀 FIX: Untuk Vercel proxy, tetap lanjut upload meskipun test gagal
      // Vercel proxy mungkin tidak merespons di /health, tapi upload endpoint tetap bekerja
      const isVercelProxy = serverUrl.includes('vercel.app') || serverUrl.includes('vercel-proxy');
      if (isVercelProxy) {
        console.error(`⚠️  Connection test failed, but continuing with Vercel proxy upload (test endpoint mungkin tidak tersedia)`);
        console.error(`✅ Using Vercel proxy: ${serverUrl}`);
      } else {
