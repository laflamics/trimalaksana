// Force Sync BOM from File to localStorage
console.log('🔄 Force syncing BOM from file to localStorage...');

// Clean BOM data from your file (the correct one)
const cleanBomData = {
  "value": [
...
// Verify problem IDs exist in clean data
const problemIds = ['KRT02722', 'KRT04173', 'KRT04072', 'KRT00199'];
console.log('\n🎯 Verifying problem IDs in clean BOM:');
problemIds.forEach(id => {
  const found = cleanBomData.value.some(item => item.product_id === id);
...
console.log('✅ Clean BOM data synced to localStorage');
...
window.dispatchEvent(new CustomEvent('bomUpdated', {
  detail: { 
    ...
    source: 'FileSyncForce'
  }
}));

console.log('✅ React reload events dispatched');
console.log('\n🔄 BOM data synced! Now run the product identifier fix...');