// Force Clean BOM - Simple and Direct
console.log('🔧 Force cleaning BOM data...');

// Clean BOM data (exact from your file)
const cleanBomItems = [
...
console.log('📊 Clean BOM items:', cleanBomItems.length);

// Verify problem IDs exist
const problemIds = ['KRT02722', 'KRT04173', 'KRT04072', 'KRT00199'];
console.log('\n🎯 Verifying problem IDs in clean data:');
problemIds.forEach(id => {
  const found = cleanBomItems.some(item => item.product_id === id);
  console.log(`${id}: ${found ? '✅ EXISTS' : '❌ MISSING'}`);
});

// Create clean BOM data structure
const cleanBomData = {
  value: cleanBomItems,
  timestamp: Date.now(),
  _timestamp: Date.now(),
...
localStorage.setItem('bom', JSON.stringify(cleanBomData)); // Also main key
console.log('✅ Clean BOM data applied');

// Verify
const verifyData = JSON.parse(localStorage.getItem(bomStorageKey));
console.log('✅ Verified BOM items:', verifyData.value.length);

// Test the exact normalization that Products.tsx will use
...
// Create Set exactly like Products.tsx will
const testBomSet = new Set();
cleanBomItems.forEach(b => {
  if (b) {
    const bomProductId = (b.product_id || b.padCode || b.kode || '').toString().trim();
    if (bomProductId) {
      const normalized = normalizeBomId(bomProductId);
      if (normalized) {
        testBomSet.add(normalized);
      }
    }
  }
});

console.log('\n🧪 Test BOM Set:', testBomSet.size);

// Test problem IDs
console.log('\n🎯 Final Test:');
problemIds.forEach(id => {
  const normalized = normalizeBomId(id);
  const found = testBomSet.has(normalized);
...
window.dispatchEvent(new CustomEvent('app-storage-changed', {
  detail: { key: bomStorageKey.split('/').pop() || bomStorageKey, value: cleanBomItems }
}));

window.dispatchEvent(new CustomEvent('bomUpdated', {
  detail: { 
    productId: 'ALL', 
    bomItems: cleanBomItems, 
    source: 'ForceClean'
  }
}));

console.log('\n🚀 DONE! Check Products page now.');
console.log('Expected: bomSetSize ~60, all problem IDs should show hasBOM: true');