...
/**
 * Convert GT CSV to JSON (salesOrders, invoices, deliveryNotes)
 * Usage: node scripts/convert-gt-csv-to-json.js <input_csv> [output_dir]
 * Example: node scripts/convert-gt-csv-to-json.js scripts/master/gt/gtfeb_cleaned.csv scripts/master/packaging/
 */
...
if (!inputFile) {
  ...
  console.error('Usage: node scripts/convert-gt-csv-to-json.js <input_csv> [output_dir]');
  process.exit(1);
}
...
const extractPoAndDo = (ket) => {
  ...
  // Extract all DO references (12xxx-xxxxxx format)
  const doMatches = ket.match(/\d{5}-\d{6}/g) || [];
  const dos = [...new Set(doMatches)];
...
rl.on('close', () => {
  ...
  const dlvFile = path.join(outputDir, 'gt_deliveryNotes.json');
