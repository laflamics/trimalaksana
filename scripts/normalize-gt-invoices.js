/**
 * Normalize GT invoices to match Packaging invoice structure
 * 
 * Transform from:
 * {
 *   "items": [{ "productCode", "productName", "qty", "unit", "price", "total", "tax", "totalAkhir" }],
 *   "discount": number,
 *   "taxPercent": number,
 *   "notes": "string"
 * }
...
function normalizeGTInvoice(invoice) {
  ...
  if (Array.isArray(invoice.items) && invoice.items.length > 0) {
    lines = invoice.items.map(item => {
      ...
      return {
        itemSku: item.productCode || item.itemSku || item.sku || item.kode || '',
...
async function normalizeGTInvoices() {
  console.log('🔄 Starting GT invoice normalization...\n');

  const filePath = 'scripts/master/packaging/gt_invoices.json';
  ...
  try {
    ...
    const normalized = invoices.map(inv => {
      const result = normalizeGTInvoice(inv);
      return result;
    }).filter(inv => inv !== null);
    ...
    console.log('\n✅ GT invoice normalization complete!');
  } catch (error) {
...
// Run
normalizeGTInvoices().catch(console.error);
