...
try {
  ...
  // Build customer lookup map (by normalized name)
  const customerLookup = new Map();
  customerMasterRecords.forEach(record => {
    const nama = record['Nama'] || record['nama'] || '';
    const kode = record['Kode'] || record['kode'] || '';
    if (nama) {
      ...
      customerLookup.set(normalized, {
        ...
        kode: kode,
        nama: nama,
        kontak: record['Kontak'] || record['kontak'] || '',
        telepon: record['Telepon'] || record['telepon'] || '',
        alamat: record['Alamat'] || record['alamat'] || '',
  ...
  const customerMap = new Map(); // customerName -> customer data (untuk tracking yang sudah di-process)
  ...
  records.forEach((record, idx) => {
    ...
    if (!customerData) {
      // Customer tidak ada di master, create baru
      ...
      // Customer ada di master, track di customerMap juga
      if (!customerMap.has(customerName)) {
        customerMap.set(customerName, customerData);
      }
    }
  ...
  console.log(`\n✓ Parsed data:`);
  console.log(`  - Customers (new + matched): ${customers.length}`);
  ...
  // Count new vs matched customers
