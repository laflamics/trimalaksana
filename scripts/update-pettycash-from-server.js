...
// This will load data from server and update Petty Cash
...
164,DO-20260123-1118,1200000,Approve,Distribusi`;
  ...
  console.log('🚀 Starting Petty Cash update from CSV (loading from server)...\n');
  ...
  const loadFromServer = async (path) => {
    try {
      const response = await fetch(`${SERVER_URL}/api/data/${path}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      return Array.isArray(data) ? data : (data.value || []);
    } catch (e) {
      console.warn(`⚠️  Failed to load ${path}:`, e.message);
      return [];
    }
  };

  const [pettyCash, deliveryOrders, drivers, vehicles, routes, deliveryNotes] = await Promise.all([
    loadFromServer('trucking/trucking_pettycash_requests'),
    loadFromServer('trucking/trucking_delivery_orders'),
    loadFromServer('trucking/trucking_drivers'),
    loadFromServer('trucking/trucking_vehicles'),
    loadFromServer('trucking/trucking_routes'),
    loadFromServer('trucking/trucking_delivery_notes'),
  ]);
  ...
  if (pettyCash.length === 0) {
    console.error('❌ No Petty Cash data found on server! Please confirm some DOs first.');
    return;
  }

  if (deliveryOrders.length === 0) {
    console.error('❌ No Delivery Orders found on server!');
    return;
  }
  ...
  try {
    const pcResponse = await fetch(`${SERVER_URL}/api/data/trucking/trucking_pettycash_requests`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(pettyCash)
    });
    
    if (!pcResponse.ok) throw new Error(`Failed to save Petty Cash: ${pcResponse.status}`);
    console.log('   ✓ Saved Petty Cash to server');
    
    const dnResponse = await fetch(`${SERVER_URL}/api/data/trucking/trucking_delivery_notes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(deliveryNotes)
    });
    
    if (!dnResponse.ok) throw new Error(`Failed to save Delivery Notes: ${dnResponse.status}`);
    console.log('   ✓ Saved Delivery Notes to server');
    ...
    console.log('⚠️  Data saved to localStorage only. Please sync manually.');
  }
