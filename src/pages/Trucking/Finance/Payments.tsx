...
const Payments = () => {
  ...
  useEffect(() => {
    ...
    const cleanup = setupRealTimeSync({
      keys: [TRUCKING_SYNC_KEYS.PAYMENTS, TRUCKING_SYNC_KEYS.PURCHASE_ORDERS, TRUCKING_SYNC_KEYS.ACCOUNTS, TRUCKING_SYNC_KEYS.FINANCE_NOTIFICATIONS],
      onUpdate: () => {
        loadPayments();
        loadPurchaseOrders();
        loadAccounts();
  ...
  const loadAccounts = async () => {
    const data = await storageService.get<any[]>(StorageKeys.TRUCKING.ACCOUNTS) || [];
  ...
  const loadPayments = async () => {
    // Load from both StorageKeys.TRUCKING.PAYMENTS (existing) and create unified list
    ...
    const invoices = await storageService.get<any[]>(StorageKeys.TRUCKING.INVOICES) || [];
    ...
    if (needsUpdate) {
      ...
      await storageService.set(StorageKeys.TRUCKING.PAYMENTS, updatedPayments);
    }
  ...
  const loadPurchaseOrders = async () => {
    const [poData, financeNotifData, paymentsData] = await Promise.all([
      ...
      storageService.get<any[]>(StorageKeys.TRUCKING.FINANCE_NOTIFICATIONS) || [],
    ...
    if (financeNotifDataArray.length > 0 && JSON.stringify(cleanedNotifs) !== JSON.stringify(financeNotifDataArray)) {
      await storageService.set(StorageKeys.TRUCKING.FINANCE_NOTIFICATIONS, cleanedNotifs);
    }
  ...
  const handleLoadNotificationToForm = async (notif: any) => {
    try {
      ...
      try {
        const invoices = await storageService.get<any[]>(StorageKeys.TRUCKING.INVOICES) || [];
        const invoicesArray = Array.isArray(invoices) ? invoices : [];
        const spks = await storageService.get<any[]>(StorageKeys.TRUCKING.SPK) || [];
