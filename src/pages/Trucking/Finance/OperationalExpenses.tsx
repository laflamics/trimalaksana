...
const OperationalExpenses = () => {
  ...
  useEffect(() => {
    ...
    const cleanup = setupRealTimeSync({
      keys: [TRUCKING_SYNC_KEYS.OPERATIONAL_EXPENSES],
      onUpdate: loadData,
    });
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const data = await storageService.get<OperationalExpense[]>(StorageKeys.TRUCKING.OPERATIONAL_EXPENSES) || [];
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingExpense) {
        ...
        await storageService.set(StorageKeys.TRUCKING.OPERATIONAL_EXPENSES, updated);
        await logUpdate('OperationalExpense', editingExpense.id, '/GeneralTrading/finance/operational-expenses', {
          expenseNo: editingExpense.expenseNo,
        ...
        await storageService.set(StorageKeys.TRUCKING.OPERATIONAL_EXPENSES, updated);
        await logCreate('OperationalExpense', newExpense.id, '/GeneralTrading/finance/operational-expenses', {
  ...
  const handleImportFromPettyCashMemo = async () => {
    try {
      setImportLoading(true);
      
      // Load PettyCash memos
      const memosRaw = await storageService.get<any[]>(StorageKeys.TRUCKING.PETTY_CASH_MEMOS) || [];
      const memos = Array.isArray(memosRaw) ? memosRaw : [];
      
      if (memos.length === 0) {
        showAlert('Tidak ada Petty Cash Memo yang tersedia untuk diimport.', 'Warning');
        return;
      }
      
      // Convert memo items to operational expenses
      const expensesArray = Array.isArray(expenses) ? expenses : [];
      const newExpenses: OperationalExpense[] = [];
      let importCount = 0;
        ...
        if (memo.items && Array.isArray(memo.items)) {
          memo.items.forEach((item: any) => {
            const newExpense: OperationalExpense = {
              ...
              no: expensesArray.length + newExpenses.length + 1,
              expenseNo: `PC-${memo.memoNo}`,
              expenseDate: memo.memoDate || new Date().toISOString().split('T')[0],
              type: 'PettyCash',
              category: 'Other',
              description: item.uraian || item.description || '',
              amount: item.nominal || 0,
              paymentMethod: 'Cash',
              requestor: memo.recipientName || '',
              notes: `From Memo: ${memo.memoNo} - ${item.ket || ''}`,
              created: new Date().toISOString(),
              lastUpdate: new Date().toISOString(),
              timestamp: Date.now(),
              _timestamp: Date.now(),
      ...
      if (importCount === 0) {
        showAlert('Tidak ada item dalam Petty Cash Memo untuk diimport.', 'Warning');
        return;
      }
      ...
      showAlert(`✅ Berhasil import ${importCount} expense(s) dari Petty Cash Memo`, 'Success');
    } catch (error: any) {
      showAlert(`Error importing from Petty Cash Memo: ${error.message}`, 'Error');
    } finally {
      setImportLoading(false);
    }
  };
            ...
            onClick={async () => {
              if (window.confirm('Delete this expense?')) {
                try {
                  ...
                  await storageService.set(StorageKeys.TRUCKING.OPERATIONAL_EXPENSES, updated);
                  await logDelete('OperationalExpense', item.id, '/GeneralTrading/finance/operational-expenses', {
    ...
    <div className="module-compact">
      <Card>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap: '10px', position: 'sticky', top: 0, backgroundColor: 'var(--bg-primary)', zIndex: 5, padding: '12px 0' }}>
          ...
          <Button variant="secondary" onClick={handleImportFromPettyCashMemo} disabled={importLoading}>
            {importLoading ? '⏳ Importing...' : '💵 Import from Petty Cash'}
          </Button>
          <Button
            onClick={() => {
              setShowForm(true);
