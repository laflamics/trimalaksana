...
const AccountsPayable = () => {
  ...
  useEffect(() => {
    ...
    const cleanup = setupRealTimeSync({
      keys: [TRUCKING_SYNC_KEYS.BILLS, TRUCKING_SYNC_KEYS.PAYMENTS, TRUCKING_SYNC_KEYS.ACCOUNTS],
      onUpdate: loadData,
    });
  ...
  const handleViewPaymentRecords = async (po: any) => {
    try {
      const payments = await storageService.get<any[]>(StorageKeys.TRUCKING.PAYMENTS) || [];
  ...
  const handlePaymentSubmit = async () => {
    try {
      ...
      await storageService.set(StorageKeys.TRUCKING.PAYMENTS, updated);
  ...
  const generateMissingJournalEntries = async () => {
    try {
      ...
      const [entriesRaw, posRaw, accountsRaw] = await Promise.all([
        ...
        storageService.get<any[]>(StorageKeys.TRUCKING.ACCOUNTS),
      ]);
      ...
      if (newEntries.length > 0) {
        await storageService.set(StorageKeys.TRUCKING.JOURNAL_ENTRIES, [...entriesArray, ...newEntries]);
        showAlert(
                      ...
                      if (file) {
                        ...
                        reader.onload = async (event) => {
                          try {
                            ...
                            const financeNotifications = await storageService.get<any[]>(StorageKeys.TRUCKING.FINANCE_NOTIFICATIONS) || [];
                            ...
                            await storageService.set(StorageKeys.TRUCKING.FINANCE_NOTIFICATIONS, updatedNotifications);
                            
                            // Update purchaseOrders status to CLOSE
                            const purchaseOrders = await storageService.get<any[]>(StorageKeys.TRUCKING.PURCHASE_ORDERS) || [];
                            ...
                            await storageService.set(StorageKeys.TRUCKING.PURCHASE_ORDERS, updatedPOs);
