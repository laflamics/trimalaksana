...
const TaxManagement = () => {
  ...
  useEffect(() => {
    ...
    const cleanup = setupRealTimeSync({
      keys: [TRUCKING_SYNC_KEYS.TAX_RECORDS],
      onUpdate: loadData,
    });
  ...
  const loadData = async () => {
    try {
      ...
      const [invsRaw, paymentsRaw, taxRecordsRaw] = await Promise.all([
        ...
        storageService.get<Payment[]>(StorageKeys.TRUCKING.PAYMENTS),
        storageService.get<TaxRecord[]>(StorageKeys.TRUCKING.TAX_RECORDS),
      ]);
      ...
      console.log('[TaxManagement] Loaded invoices:', Array.isArray(invs) ? invs.length : 0);
      console.log('[TaxManagement] Loaded payments:', Array.isArray(pmts) ? pmts.length : 0);
      ...
      (Array.isArray(invs) ? invs : []).forEach((inv: any) => {
        ...
        console.log(`[TaxManagement] Invoice ${inv.invoiceNo}: tax=${taxAmount}, bom.tax=${inv.bom?.tax}, inv.tax=${inv.tax}`);
      ...
      console.log('[TaxManagement] Generated tax records from invoices:', records.length);
      ...
      console.log('[TaxManagement] Final tax records:', finalRecords.length);

      // Save to storage for AllReportsFinance to read
      await storageService.set(StorageKeys.TRUCKING.TAX_RECORDS, finalRecords);

      setTaxRecords(finalRecords);
    } catch (error: any) {
      console.error('[TaxManagement] Error loading data:', error);
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      // Save to storage
      await storageService.set(StorageKeys.TRUCKING.TAX_RECORDS, updated);
      
      // Log the payment
      await logUpdate('TaxRecord', selectedRecord.id, '/GeneralTrading/finance/tax-management', {
