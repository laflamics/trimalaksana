...
const ServerData = () => {
  ...
  // Load list of storage keys for General Trading
  const loadStorageList = async () => {
    setLoading(true);
    try {
      // Get all General Trading keys
      const gtKeys = [
        ...
        StorageKeys.GENERAL_TRADING.SPK,
        StorageKeys.GENERAL_TRADING.SCHEDULE,
        StorageKeys.GENERAL_TRADING.PURCHASE_REQUESTS,
        StorageKeys.GENERAL_TRADING.PURCHASE_ORDERS,
        StorageKeys.GENERAL_TRADING.GRN,
        StorageKeys.GENERAL_TRADING.DELIVERY,
        StorageKeys.GENERAL_TRADING.INVENTORY,
        StorageKeys.GENERAL_TRADING.INVOICES,
        StorageKeys.GENERAL_TRADING.PAYMENTS,
        StorageKeys.GENERAL_TRADING.JOURNAL_ENTRIES,
        StorageKeys.GENERAL_TRADING.ACCOUNTS,
        StorageKeys.GENERAL_TRADING.EXPENSES,
        StorageKeys.GENERAL_TRADING.TAX_RECORDS,
        StorageKeys.GENERAL_TRADING.DELIVERY_NOTIFICATIONS,
        StorageKeys.GENERAL_TRADING.INVOICE_NOTIFICATIONS,
        StorageKeys.GENERAL_TRADING.FINANCE_NOTIFICATIONS,
        StorageKeys.GENERAL_TRADING.PURCHASING_NOTIFICATIONS,
      ];
      ...
      for (const key of gtKeys) {
        try {
          ...
          if (data && Array.isArray(data) && data.length > 0) {
            ...
            console.log(`[GT ServerData] ✅ Found ${key}: ${data.length} items`);
          }
        } catch (error) {
      ...
      console.log('[GT ServerData] ✅ Loaded storage list:', items.length, 'keys with data');
    } catch (error: any) {
      showAlert(`Error loading storage list: ${error.message}`, 'Error');
      console.error('[GT ServerData] Error loading storage list:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadKeyData = async (key: string) => {
    ...
    try {
      ...
      if (data && Array.isArray(data)) {
        setKeyData(data);
        setSelectedKey(key);
        console.log('[GT ServerData] ✅ Loaded data for key:', key, '- Items:', data.length);
      } else {
        setKeyData([]);
        setSelectedKey(key);
        console.log('[GT ServerData] ⚠️ No data for key:', key);
      }
    } catch (error: any) {
      showAlert(`Error loading data for ${key}: ${error.message}`, 'Error');
      console.error('[GT ServerData] Error loading key data:', error);
    } finally {
      setKeyLoading(false);
    }
  };
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          ...
          console.error('[GT ServerData] Error deleting items:', error);
        } finally {
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          ...
          console.error('[GT ServerData] Error deleting key:', error);
        } finally {
          setDeleteLoading(false);
        }
      }
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          ...
          console.error('[GT ServerData] Error clearing key:', error);
        } finally {
          setDeleteLoading(false);
        }
      }
  ...
  return (
    <div style={{ padding: '20px' }}>
      <Card>
        <h1>General Trading - Server Data Management</h1>
        <p style={{ color: '#666', marginBottom: '20px' }}>
          View and manage General Trading data stored on the server
        </p>
        ...
        <div style={{ marginBottom: '30px' }}>
          ...
          {storageItems.length > 0 ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f5f5f5', borderBottom: '1px solid #ddd' }}>
                    <th style={{ padding: '10px', textAlign: 'left', fontWeight: 'bold' }}>Storage Key</th>
                    <th style={{ padding: '10px', textAlign: 'left', fontWeight: 'bold' }}>Items</th>
                    <th style={{ padding: '10px', textAlign: 'left', fontWeight: 'bold' }}>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {storageItems.map((item) => (
                    <tr
                      ...
                      style={{
                        ...
                        backgroundColor: selectedKey === item.key ? '#e3f2fd' : 'white',
                      }}
                    >
            ...
            <p style={{ color: '#999' }}>No storage keys found</p>
          )}
        </div>
        ...
        {selectedKey && (
          <div>
            ...
            {keyData.length > 0 ? (
              <>
                ...
                <div style={{ overflowX: 'auto', maxHeight: '600px', overflowY: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#f5f5f5', borderBottom: '1px solid #ddd' }}>
                        <th style={{ padding: '10px', textAlign: 'left', fontWeight: 'bold', width: '50px' }}>
                          <input
                            type="checkbox"
              ...
              <p style={{ color: '#999' }}>No data in this key</p>
            )}
          </div>
        )}
      </Card>
      ...
      {dialogState.show && (
        ...
        >
          ...
          >
            <h3 style={{ margin: '0 0 10px 0' }}>{dialogState.title}</h3>
            <p style={{ margin: '0 0 20px 0', whiteSpace: 'pre-wrap' }}>{dialogState.message}</p>
