...
const Accounting = () => {
  ...
  const handleCreateInvoice = async (item: any) => {
    ...
    try {
      ...
      await storageService.set('gt_invoices', updated);
      setInvoices(updated);
        ...
        try {
          ...
          if (cogsAmount > 0) {
            ...
            if (!hasCOGSEntry) {
              ...
              const currentEntries = await storageService.get<any[]>('gt_journalEntries') || [];
      ...
      if (notif) {
        ...
        await storageService.set('gt_invoiceNotifications', updatedNotifs);
  ...
  const getExpenseRowColor = (expenseNo: string): string => {
    ...
    if (isLightMode) {
      ...
      return expenseIndex % 2 === 0 ? (bgSecondary || '#1e1e1e') : (bgTertiary || '#2a2a2a'); // Dark mode: gunakan CSS variables
    }
  };
        ...
        <UpdateInvoiceDialog
          ...
          onSave={async (paymentProof: string, paymentProofName: string) => {
            ...
            try {
              ...
              const existingPayments = await storageService.get<any[]>('gt_payments') || [];
              ...
              if (!existingPayment) {
                ...
                await storageService.set('gt_payments', [...existingPayments, newPayment]);
              }
            } catch (error: any) {
            ...
            if (updatingInvoice.soNo) {
              ...
              await storageService.set('gt_salesOrders', updatedSOs);
...
// Create Invoice Dialog Component untuk GT
  ...
  useEffect(() => {
    if (selectedSOs.length > 0 && mode === 'so') {
      const loadSOData = async () => {
        const soData = await storageService.get<any[]>('gt_salesOrders') || [];
        ...
        if (selectedSOItems.length > 0) {
          ...
          const custData = await storageService.get<any[]>('gt_customers') || [];
          ...
          selectedSOItems.forEach((soItem: any) => {
            ...
            items.forEach((item: any) => {
              ...
              allItems.push({
                ...
                price: price,
                unit: item.unit || 'PCS',
                soNo: soItem.soNo || '', // Simpan SO number jika ada
              });
            });
          });
  ...
  useEffect(() => {
    if (selectedSJs.length > 0 && mode === 'sj') {
      const loadDNData = async () => {
        const dnData = await storageService.get<any[]>('gt_delivery') || [];
        ...
        if (selectedDNItems.length > 0) {
          ...
          // Gabungkan notes dari semua DN dengan format DN: SJ-xxx
          const notes = selectedDNItems.map((dnItem: any) => {
            return dnItem.sjNo ? `DN: ${dnItem.sjNo}` : '';
          }).filter((note: string) => note).join(' ');
              ...
              <div style={{ 
                ...
                <div style={{ maxHeight: '100px', overflowY: 'auto', fontSize: '10px' }}>
                  {autoDNItems.map((item, idx) => {
                    ...
                    const productDisplay = item.soNo ? `${item.product} (SO: ${item.soNo})` : item.product;
                    return (
                      <div key={idx} style={{ padding: '2px 0', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                        {productDisplay} - Qty: {item.qty} {item.unit || 'PCS'} - Price: Rp {item.price.toLocaleString('id-ID')} - Total: Rp {totalItem.toLocaleString('id-ID')}
                      </div>
                    );
                  })}
                </div>
