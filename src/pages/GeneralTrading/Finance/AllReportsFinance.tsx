...
const AllReportsFinance = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [invsRaw, posRaw, paymentsRaw, taxRaw, opsRaw, invRaw] = await Promise.all([
        ...
        storageService.get<any[]>(StorageKeys.GENERAL_TRADING.TAX_RECORDS),
        storageService.get<any[]>(StorageKeys.GENERAL_TRADING.OPERATIONAL_EXPENSES),
        storageService.get<any[]>(StorageKeys.GENERAL_TRADING.INVENTORY),
      ]);
      ...
      if (!invs || invs.length === 0) {
        const fileData = await storageService.forceReloadFromFile<any[]>(StorageKeys.GENERAL_TRADING.INVOICES);
  ...
  // Calculate AR Close (Dana Masuk) - from CLOSE status invoices
  const arClose = useMemo(() => {
    return (Array.isArray(invoices) ? invoices : [])
      .filter(inv => {
        const invDate = inv.created || '';
        const dateOnly = invDate.split('T')[0];
        return inv.status === 'CLOSE' && dateOnly >= startDate && dateOnly <= endDate;
      })
      .reduce((sum, inv) => sum + (inv.paidAmount || 0), 0);
  }, [invoices, startDate, endDate]);

  // Calculate AR Open (Outstanding Invoice) - from OPEN status invoices
  const arOpen = useMemo(() => {
    return (Array.isArray(invoices) ? invoices : [])
      ...
      .reduce((sum, inv) => {
        // Calculate invoice total from items if not available at root
        let invoiceTotal = inv.total || 0;
        if (invoiceTotal === 0 && Array.isArray(inv.items)) {
          invoiceTotal = inv.items.reduce((itemSum: number, item: any) => itemSum + (item.totalAkhir || 0), 0);
        }
        const balance = invoiceTotal - (inv.paidAmount || 0);
        return sum + balance;
      }, 0);
  }, [invoices, startDate, endDate]);
