...
const GeneralLedger = () => {
  ...
  const loadEntries = async () => {
    ...
    if (data.length === 0) {
      ...
      const reloadedData = await storageService.get<JournalEntry[]>('gt_journalEntries') || [];
      data = Array.isArray(reloadedData) ? reloadedData : [];
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingEntry) {
        ...
        await storageService.set('gt_journalEntries', updated);
  ...
  const handleImportExcel = () => {
    ...
    const exampleHeaders = ['Date', 'Reference', 'Account Code', 'Account Name', 'Description', 'Debit', 'Credit'];
    const exampleData = [
      { 'Date': '2024-01-15', 'Reference': 'REF-001', 'Account Code': '1000', 'Account Name': 'Cash', 'Description': 'Contoh entry debit', 'Debit': 1000000, 'Credit': 0 },
      { 'Date': '2024-01-15', 'Reference': 'REF-001', 'Account Code': '4000', 'Account Name': 'Sales Revenue', 'Description': 'Contoh entry credit', 'Debit': 0, 'Credit': 1000000 },
    ];
