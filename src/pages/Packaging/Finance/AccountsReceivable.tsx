...
const AccountsReceivable = () => {
  ...
  const openColumns = useMemo(() => [
    { key: 'invoiceNo', header: t('finance.invoiceNumber') || 'Invoice No' },
    { key: 'invoiceDate', header: t('finance.invoiceDate') || 'Invoice Date' },
    { key: 'dueDate', header: t('finance.dueDate') || 'Due Date' },
    { key: 'customerName', header: t('master.customerName') || 'Customer' },
    { key: 'soNo', header: 'SO No' },
    { key: 'invoiceTotal', header: t('common.amount') || 'Invoice Amount', render: (item: any) => {
      const invoiceTotal = item.invoiceTotal || 0;
      return `Rp ${invoiceTotal.toLocaleString('id-ID')}`;
    }},
    { key: 'paidAmount', header: t('finance.paid') || 'Paid', render: (item: any) => `Rp ${(item.paidAmount || 0).toLocaleString('id-ID')}` },
    ...
    { key: 'status', header: t('common.status') || 'Status', render: (item: any) => item.isOverdue ? 'Overdue' : 'Current' },
    {
      key: 'proof',
      header: 'Proof',
      render: (item: any) => (
  ...
  const closeColumns = useMemo(() => [
    ...
    { key: 'status', header: t('common.status') || 'Status', render: () => (
