/**
 * GT Delete Helper
 * Helper terpusat untuk delete operations di General Trading business module
 ...
 */
export async function deleteGTItem(
  storageKey: string,
  ...
  try {
    ...
    // 🚀 TOMBSTONE: Dispatch custom event untuk GT delete
    window.dispatchEvent(new CustomEvent('gt-item-deleted', {
      detail: {
        storageKey: storageKey,
        itemId: itemId,
        idField: idField,
 ...
 */
export async function deleteGTItems(
  storageKey: string,
  ...
  const deletePromises = itemIds.map(async (itemId) => {
    const result = await deleteGTItem(storageKey, itemId, idField);
    if (result.success) {
      results.success++;
    } else {
      results.failed++;
 ...
 */
export async function reloadGTData<T extends Record<string, any>>(
  storageKey: string,
 ...
 */
export async function safeDeleteGTItem(
  storageKey: string,
  itemId: string | number,
  dependencies: {
    storageKey: string;
  ...
  // Delete item
  return await deleteGTItem(storageKey, itemId, idField);
}