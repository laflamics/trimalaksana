...
export function generateInvoiceHtml({
  ...
  try {
    ...
    // Default: template1 (template yang sudah ada)

    // Safety check for inv
    if (!inv) {
  ...
  // Hitung tanggal jatuh tempo dari createdAt + topDays
  let tanggalJt = bom.tanggalJt || '';
  if (!tanggalJt && inv.createdAt) {
    const createdDate = new Date(inv.createdAt);
    const topDays = bom.topDays || 30; // Default 30 hari jika tidak ada
    const dueDate = new Date(createdDate);
    dueDate.setDate(dueDate.getDate() + topDays);
    tanggalJt = dueDate.toISOString();
  }
  ...
  // Format tanggal jatuh tempo (dd/mm/yyyy)
  const formattedTanggalJt = tanggalJt ? (() => {
    const date = new Date(tanggalJt);
  ...
  // Split customer address menjadi 3 baris (sama seperti di surat jalan)
  ...
  // Split company address menjadi 3 baris (sama seperti customer address)
  ...
  if (companyFullAddress) {
    ...
    const isVeryLongAddress = companyFullAddress.length > 50;

    // Cari posisi split yang tepat
    const kabupatenIndex = companyFullAddress.toLowerCase().indexOf('kabupaten');
    const lastCommaIndex = companyFullAddress.lastIndexOf(',');
    ...
    if (isVeryLongAddress && words.length > 10) {
      ...
      const thirdPoint = Math.ceil(words.length / 3);
      const twoThirdPoint = Math.ceil(words.length * 2 / 3);
      companyAddressLine1 = words.slice(0, thirdPoint).join(' ');
      companyAddressLine2 = words.slice(thirdPoint, twoThirdPoint).join(' ');
      companyAddressLine3 = words.slice(twoThirdPoint).join(' ');
    } else if (kabupatenIndex !== -1) {
      // Split di "Kabupaten" - baris 1 sampai sebelum "Kabupaten", baris 2 dari "Kabupaten"
      companyAddressLine1 = companyFullAddress.substring(0, kabupatenIndex).trim();
      const afterKabupaten = companyFullAddress.substring(kabupatenIndex).trim();
      // Jika setelah kabupaten masih panjang, bagi lagi
      if (afterKabupaten.length > 40 && isVeryLongAddress) {
        const midPoint = Math.ceil(afterKabupaten.length / 2);
        const spaceIndex = afterKabupaten.indexOf(' ', midPoint);
        if (spaceIndex !== -1) {
          companyAddressLine2 = afterKabupaten.substring(0, spaceIndex).trim();
          companyAddressLine3 = afterKabupaten.substring(spaceIndex).trim();
        } else {
          companyAddressLine2 = afterKabupaten;
          companyAddressLine3 = '';
        }
      } else {
        companyAddressLine2 = afterKabupaten;
    ...
    } else if (lastCommaIndex !== -1 && (lastCommaIndex > companyFullAddress.length * 0.3 || isLongAddress)) {
      // Split di koma terakhir
      companyAddressLine1 = companyFullAddress.substring(0, lastCommaIndex).trim();
      const afterComma = companyFullAddress.substring(lastCommaIndex + 1).trim();
      // Jika setelah koma masih panjang, bagi lagi
      if (afterComma.length > 40 && isVeryLongAddress) {
        const midPoint = Math.ceil(afterComma.length / 2);
        const spaceIndex = afterComma.indexOf(' ', midPoint);
        if (spaceIndex !== -1) {
          companyAddressLine2 = afterComma.substring(0, spaceIndex).trim();
          companyAddressLine3 = afterComma.substring(spaceIndex).trim();
        } else {
          companyAddressLine2 = afterComma;
          companyAddressLine3 = '';
        }
      } else {
        companyAddressLine2 = afterComma;
      ...
      if (words.length > 10 && isVeryLongAddress) {
        ...
        const midPoint = Math.ceil(companyFullAddress.length / 2);
        const spaceIndex = companyFullAddress.indexOf(' ', midPoint);
        if (spaceIndex !== -1) {
          companyAddressLine1 = companyFullAddress.substring(0, spaceIndex).trim();
          companyAddressLine2 = companyFullAddress.substring(spaceIndex).trim();
          companyAddressLine3 = '';
        } else {
          companyAddressLine1 = companyFullAddress.substring(0, midPoint).trim();
          companyAddressLine2 = companyFullAddress.substring(midPoint).trim();
          companyAddressLine3 = '';
        }
      }
    } else {
    ...
    // Validasi final: pastikan semua bagian alamat ada
    const combinedAddress = (companyAddressLine1 + ' ' + companyAddressLine2 + ' ' + companyAddressLine3).trim().replace(/\s+/g, ' ');
    const normalizedFull = companyFullAddress.replace(/\s+/g, ' ').trim();
    if (combinedAddress.length < normalizedFull.length * 0.95) {
      // Jika ada yang hilang lebih dari 5%, gunakan alamat lengkap di baris 1
      companyAddressLine1 = companyFullAddress;
      companyAddressLine2 = '';
    ...
    <div class="header-top">
      ...
      <div class="header-right">
        ...
        <div class="company-address">
          ${companyAddressLine1 ? `<span class="company-address-line">${companyAddressLine1}</span>` : ''}
          ${companyAddressLine2 ? `<span class="company-address-line">${companyAddressLine2}</span>` : ''}
...
// Template 2: Layout baru sesuai spesifikasi
function generateInvoiceHtmlTemplate2({
  logo,
  company,
  inv,
  customerAddress = '',
  ...
  const formatTerbilangWithMinWords = (text: string, minWordsPerLine: number = 6): string => {
    const words = text.split(' ');
    if (words.length <= minWordsPerLine) {
      return text; // Jika kurang dari minWordsPerLine, return as is
    }

    // Gabungkan kata-kata menjadi kelompok dengan minimal minWordsPerLine kata per baris
    const lines: string[] = [];
    for (let i = 0; i < words.length; i += minWordsPerLine) {
      const lineWords = words.slice(i, Math.min(i + minWordsPerLine, words.length));
      // Gunakan non-breaking space untuk kata-kata dalam satu baris
      const line = lineWords.join('\u00A0'); // Non-breaking space
      lines.push(line);
    }

    // Gabungkan dengan <br/> untuk baris baru
    return lines.join('<br/>');
  };

  const terbilangText = terbilang(Math.floor(total)).charAt(0).toUpperCase() + terbilang(Math.floor(total)).slice(1) + ' Rupiah';
  const totalTerbilang = formatTerbilangWithMinWords(terbilangText, 6);
  ...
  // Hitung tanggal jatuh tempo dari createdAt + topDays
  let tanggalJtDate = bom.tanggalJt || '';
  if (!tanggalJtDate && inv.createdAt) {
    const createdDate = new Date(inv.createdAt);
    const topDays = bom.topDays || 30; // Default 30 hari jika tidak ada
    const dueDate = new Date(createdDate);
    dueDate.setDate(dueDate.getDate() + topDays);
    tanggalJtDate = dueDate.toISOString();
  }
  
  // Format tanggal jatuh tempo (dd/mm/yyyy)
  const tanggalJt = tanggalJtDate ? (() => {
    const date = new Date(tanggalJtDate);
    const day = String(date.getDate()).padStart(2, '0');
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const year = date.getFullYear();
    return `${day}/${month}/${year}`;
  ...
  // Split customer address menjadi 3 baris (sama seperti template 1)
  let customerAddressLine1 = '';
  let customerAddressLine2 = '';
  let customerAddressLine3 = '';
...
<style>
  ...
  * {
  ...
  .terbilang-content {
    ...
    flex: 1;
    word-spacing: 2px;
    /* Pastikan minimal 3-4 kata per baris sebelum wrap */
    min-width: 0;
  ...
  <div class="header-container">
    <div class="header-left">
      <div class="logo-container">
        ${logo ? `<img src="${logoSrc}" alt="Logo" />` : ''}
      </div>
      <div class="company-info">
        <div class="company-name">${company.companyName || 'PT TRIMA LAKSANA JAYA PRATAMA'}</div>
        <div class="company-address">
          ${company.address || 'Jl. Raya Cikarang Cibarusah Kp. Kukun RT 11/06 Desa Ciantra Kecamatan Cikarang Selatan Kabupaten Bekasi'}<br/>
    ...
    <div class="footer-top">
      <div class="footer-left">
        <div class="terbilang-section">
          ...
          <div class="terbilang-content">${totalTerbilang}</div>
        </div>
      ...
      (() => {
        ...
        if (poMatches.length > 0 || doMatches.length > 0) {
          ...
          let html = '';
          poMatches.forEach((po: string) => {
            html += `<div style="margin: 2px 0;">${po}</div>`;
          });
          doMatches.forEach((do_: string) => {
            html += `<div style="margin: 2px 0;">${do_}</div>`;
          });
          return html;
        }
